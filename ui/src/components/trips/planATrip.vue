    ...
    <div class="container">
        ...
        <b-modal ref="editModal" id="editModal" hide-footer title="Edit Destination">
            <div class="d-block">
                <b-form-group id="editInDate-field" label="In Date:" label-for="editInDate">
                    <b-input id="editInDate" :type="'date'" v-model="editInDate" max='9999-12-31'>{{editInDate}} trim
                    </b-input>
                </b-form-group>
                <b-form-group id="editOutDate-field" label="Out Date:" label-for="editOutDate">
                    <b-input id="editOutDate" :type="'date'" v-model="editOutDate" max='9999-12-31'>{{editOutDate}}
                        trim
                    </b-input>
                </b-form-group>
            </div>
            <b-button class="mr-2 float-right" variant="success"
                      @click="saveDestination(rowEdit, editInDate, editOutDate); dismissModal; dismissCountDown">Save
            </b-button>
                    ...
                    <b-row>
                        <b-col>
                            <b-form-group
                                ...
                                <b-form-select id="destination" v-model="tripDestination" :type="'text'" trim>
                                    <option v-for="destination in destinations" :value="destination">
                                        {{destination.name}}
                                    </option>
                                </b-form-select>
                            </b-form-group>
                        ...
                        <b-col>
                            <b-form-group
                                    ...
                                    label-for="inDate">
                                <b-form-input id="inDate" v-model="inDate" :type="'date'" max='9999-12-31'
                                              trim></b-form-input>
                            </b-form-group>
                        </b-col>
                        <b-col>
                            <b-form-group
                                    ...
                                    label-for="outDate">
                                <b-form-input id="outDate" v-model="outDate" :type="'date'" max='9999-12-31'
                                              trim></b-form-input>
                            </b-form-group>
                        </b-col>
                    </b-row>
                    <b-button class="mr-2 float-right" variant="primary" @click="checkDestination">Add Destination
                    </b-button>
                </b-container>
        ...
        >
            ...
            <template slot="order" slot-scope="row">
                <b-button size="sm" :disabled="tripDestinations.length === 1 || row.index === 0"
                          @click="moveUpCheck(row.index)" variant="success" class="mr-2">&uarr;
                </b-button>
                <b-button size="sm" :disabled="tripDestinations.length === 1 || row.index === tripDestinations.length-1"
                          @click="moveDownCheck(row.index)" variant="success" class="mr-2">&darr;
                </b-button>
            </template>
        ...
        <b-button variant="primary" block class="mr-2 float-right" @click="submitTrip">
            <b-spinner small v-if="savingTrip" variant="dark" label="Spinning">Saving...</b-spinner>
            Save Trip
        </b-button>
        ...
        methods: {
            ...
            moveUpCheck(rowIndex) {
                if (this.tripDestinations.length === 2) {
                    ...
                    && this.tripDestinations[rowIndex - 1].destId === this.tripDestinations[rowIndex + 1].destId) {
            ...
            moveDownCheck(rowIndex) {
                if (this.tripDestinations.length === 2) {
                    ...
                    && this.tripDestinations[rowIndex + 2].destId !== this.tripDestinations[rowIndex].destId) {
                    this.moveDown(rowIndex);
                }
                else if (rowIndex === 0
                    && this.tripDestinations[rowIndex + 2].destId === this.tripDestinations[rowIndex].destId) {
            ...
            showDuplicateDestError(error) {
                this.showError = true;
                this.errorMessage =
                    "Can't have same destination next to another, please choose another destination to " + error;
            },
                ...
                if (this.tripName === null || this.tripName.length === 0) {
                    ...
                    for (let i = 0; i < this.tripDestinations.length; i++) {
                        if (this.tripDestinations[i].in_date === undefined
                            || this.tripDestinations[i].in_date.length === 0) {
                            this.tripDestinations[i].in_date = null;
                        }
                        if (this.tripDestinations[i].out_date === undefined
                            || this.tripDestinations[i].out_date.length === 0) {
